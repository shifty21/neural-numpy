...
def cnn(weights):
    conv = ConvolutionalLayer(2, kernel_size=5, init_func=f.zero, act_func=f.sigmoid)
    fcl = FullyConnectedLayer(height=10, init_func=f.zero, act_func=f.softmax)

    net = n.NeuralNetwork([
        InputLayer(height=28, width=28),
        conv,
        MaxPoolingLayer(pool_size=3),
        fcl
