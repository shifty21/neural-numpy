    ...
    def train(self,
        ...
        for i in range(num_epochs):
            ...
            if vld_set:
                ...
                u.print(
                    "Epoch %02d %s [%d/%d] > Testing \n" %
                    (i + 1, u.bar(inputs_done, len(inputs)), inputs_done,
                     len(inputs)),
                    override=True)
                accuracy = self.test(net, vld_set)
                u.print(
                    "Epoch %02d %s [%d/%d] > Validation \n" %
                    (i + 1, u.bar(inputs_done, len(inputs)), inputs_done,
                     len(inputs)),
                    override=True)
    ...
    def save(self, net, filename, convert_to_float=False):
        with open(filename, "wb") as f:
            ...
            for prev_layer, layer in net.layers:
                ...
                if len(weights) > 0:
                    self.log.debug("printing weights for layer: " +
                                   str(prev_layer) + " weights shape " +
                                   str(weights.shape))
                    w.append(weights)
                if len(biases) > 0:
                    self.log.debug("printing biases for layer: " +
                                   str(prev_layer) + " biases shape " +
                                   str(biases.shape))
                    b.append(biases)
                last_layer = layer
            ...
            if len(weights) > 0:
                self.log.debug("printing weights for last layer: " +
                               str(layer) + " weights shape " +
                               str(weights.shape))
                w.append(weights)
            if len(biases) > 0:
                self.log.debug("printing biases for last layer: " +
                               str(layer) + " biases shape " +
                               str(biases.shape))
                b.append(biases)
            np.savez_compressed(f, w=w, b=b)
            self.log.debug("length of weights is " + str(len(w)) +
                           " length of biases is " + str(len(b)))
